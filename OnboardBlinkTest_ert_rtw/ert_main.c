...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (OnboardBlinkTest_M->Timing.clockTick0
    +OnboardBlinkTest_M->Timing.clockTickH0* 4294967296.0);
...
int main(void)
{
  ...
  errorCode = extmodeInit(OnboardBlinkTest_M->extModeInfo, &rtmGetTFinal
    (OnboardBlinkTest_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(OnboardBlinkTest_M, true);
    }
  }
